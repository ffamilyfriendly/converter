...
class Option {
    ...
    toObject() {
        ...
        if(this.type != OptionTypeEnum["SUB_COMMAND"]) returnValue.choices = Array.from(this.choices.entries()) 
...
const mapToArray = (key, value) => {
    if(value instanceof Map) {
        let rv = []
        for(const [name, v] of value)
            rv.push(v)
        return rv
...
class InteractionHandler {
    ...
    register(command, id) {
        ...
        return new Promise((resolve, reject) => {
            console.log(JSON.stringify(command.options,mapToArray),)
            req(apipath, {
                method: "POST",
                body: JSON.stringify(command.options,mapToArray),
                headers: {
...
server.post("/dev/interaction", (req,res) => {
